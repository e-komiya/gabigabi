...
const MainScreen = () => {
  ...
  const [processingAction, setProcessingAction] = useState<
    'gabigabi' | 'convert' | 'targetSize' | null
  >(null);
  const [targetSizeNotReached, setTargetSizeNotReached] = useState<{
    target: number;
    actual: number;
  } | null>(null);
  ...
  // Custom hooks
  const { saveMessage, saveMessageOpacity, showSaveFeedback } =
    useSaveFeedback();
  const {
    notifyProcessingStarted,
    notifyProcessingUpdate,
    notifyProcessingResult,
  } = useProcessingNotification();
  ...
  const handleProcess = async () => {
    ...
    let processingNotificationId = await notifyProcessingStarted(
      t('notifyStarted'),
    );
    try {
      ...
      processingNotificationId = await notifyProcessingUpdate(
        t('notifyInputAnalyzed'),
        processingNotificationId,
      );

      if (selectedMediaType === 'video') {
        processingNotificationId = await notifyProcessingUpdate(
          t('notifyVideoProcessing'),
          processingNotificationId,
        );
        ...
        if (gabigabiLevel === null) {
          ...
          if (resizePercent === 100 && outputFormat === 'jpeg') {
            Alert.alert(
              t('noConversionNeededTitle'),
              t('noConversionNeededMessage'),
            );
            return;
          }
  ...
  const handleTargetSizeProcess = async () => {
    ...
    let processingNotificationId = await notifyProcessingStarted(
      t('notifyCompressStarted'),
    );
    try {
      const inputBytes = getFileSizeBytes(selectedImage);
      processingNotificationId = await notifyProcessingUpdate(
        t('notifyCompressProcessing'),
        processingNotificationId,
      );
      ...
      setTargetSizeNotReached(
        result.outputBytes > targetBytes
          ? { target: targetBytes, actual: result.outputBytes }
          : null,
      );
      await saveHistory(
        'targetSize',
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {saveMessage && (
        <Animated.View
          style={[styles.saveFeedback, { opacity: saveMessageOpacity }]}
        >
      ...
      >
            ...
            {targetSizeNotReached && (
              <View style={styles.targetSizeWarning} accessibilityRole="alert">
                <Text style={styles.targetSizeWarningText}>
                  ⚠️ {t('targetSizeNotReachedPrefix')}
                  {formatBytes(targetSizeNotReached.target)}
                  {t('targetSizeNotReachedMiddle')}
                  {formatBytes(targetSizeNotReached.actual)}
                  {t('targetSizeNotReachedSuffix')}
                </Text>
              </View>
            )}
          </View>
        ...
        <View style={styles.methodTabs}>
          <TouchableOpacity
            style={[
              styles.methodTab,
              convertMethod === 'parameters' && styles.methodTabActive,
            ]}
          ...
          >
            <Text
              style={[
                ...
                convertMethod === 'parameters' && styles.methodTabTextActive,
              ]}
            >
              {t('setParameters')}
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.methodTab,
              convertMethod === 'targetSize' && styles.methodTabActive,
            ]}
          ...
          >
            <Text
              style={[
                styles.methodTabText,
                convertMethod === 'targetSize' && styles.methodTabTextActive,
              ]}
            >
              {t('setTargetSize')}
            </Text>
      ...
      >
        ...
        {convertMethod === 'parameters' ? (
          ...
          <TouchableOpacity
            style={[
              styles.targetSizeProcessButton,
              (!selectedImage || isProcessing) && styles.disabledButton,
            ]}
