...
jest.mock('../data/ffmpeg/ffmpegUtils', () => ({
  buildFfmpegCommand: jest
    .fn()
    .mockImplementation((args: string[]) => args.filter(Boolean).join(' ')),
...
function setupSuccessSession() {
  ...
  ffmpegUtils.buildFfmpegCommand.mockImplementation((args: string[]) =>
...
describe('convertImage', () => {
  ...
  it('JPEG変換時に -q:v を使う', async () => {
    ...
    const result = await convertImage('file:///photos/in.png', {
      outputFormat: 'jpeg',
      quality: 99,
    });
  ...
  it.each([
    ...
    '主要フォーマット変換を網羅できる: $format',
    async ({ format, expectedExt, qualityArg }) => {
      mockGetFileSizeBytes.mockReturnValueOnce(1000).mockReturnValue(600);

      const result = await convertImage('file:///photos/input.png', {
        outputFormat: format as 'jpeg' | 'png' | 'webp' | 'bmp',
      });
      const cmd = mockExecute.mock.calls[0][0] as string;

      expect(result.outputUri).toMatch(
        new RegExp(`${expectedExt.replace('.', '\\.')}$`),
      );
      if (qualityArg) {
  ...
  it('GIF変換時に2パス実行しパレットを削除する', async () => {
    ...
    const result = await convertImage('file:///photos/in.mp4', {
      outputFormat: 'gif',
    });
  ...
  it('GIF変換時にfpsとscaleオプションを反映する', async () => {
    ...
    await convertImage('file:///photos/in.mp4', {
      outputFormat: 'gif',
      gifFps: 15,
      gifScale: 50,
    });
  ...
  it('入力ファイルが存在しない場合はエラー', async () => {
    ...
    await expect(
      convertImage('file:///photos/in.jpg', { outputFormat: 'webp' }),
  ...
  it('FFmpeg失敗時に出力を削除してエラー', async () => {
    ...
    expect(mockFileDelete).toHaveBeenCalledWith(
      'file:///cache/in_converted_12345_abc.webp',
    );
  });
  ...
  it.each([
    { outputFormat: 'jpeg' as const, expectedExt: '.jpg', expectedArg: '-q:v' },
    {
      ...
      expectedExt: '.png',
      expectedArg: '-compression_level 6',
    },
    {
    ...
    {
      outputFormat: 'bmp' as const,
      expectedExt: '.bmp',
      expectedArg: '-frames:v 1',
    },
  ])(
    '主要フォーマット変換($outputFormat)で拡張子とコマンドが一致する',
    async ({ outputFormat, expectedExt, expectedArg }) => {
      ...
      expect(result.outputUri).toBe(
        `file:///cache/input_converted_12345_abc${expectedExt}`,
      );
  ...
  it('GIF変換で出力拡張子と2-pass経路が一致する', async () => {
    ...
    const result = await convertImage('file:///photos/anim.webp', {
      outputFormat: 'gif',
    });
